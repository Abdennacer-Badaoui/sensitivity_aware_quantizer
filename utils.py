        ...
        if "benchmark_results" in model_results:
            for model_type in ["original", "quantized"]:
                if model_type in model_results["benchmark_results"]:
                    for task_name, task_results in model_results["benchmark_results"][
                        model_type
                    ].items():
...
def get_metric_display_info(metric_name):
    ...
    if any(corr_metric in metric_name_lower for corr_metric in correlation_metrics):
        ...
        return {
            ...
            "multiplier": 100
            if "accuracy" in metric_name_lower and metric_name_lower != "accuracy"
            else 1,
            "format": ".1f",
            "higher_better": True,
            "range": None,
...
def plot_comparisons(
    ...
    benchmark_colors = {"original": "tab:green", "quantized": "tab:olive"}
    ...
    for task_idx, (task_name, metrics) in enumerate(tasks_metrics.items()):
        for metric_idx, metric_name in enumerate(metrics):
            ...
            for model_name in model_names:
                ...
                orig_score = 0
                if (
                    "benchmark_results" in results
                    and "original" in results["benchmark_results"]
                    and task_name in results["benchmark_results"]["original"]
                    and metric_name
                    in results["benchmark_results"]["original"][task_name]
                ):
                    orig_score = results["benchmark_results"]["original"][task_name][
                        metric_name
                    ]
                ...
                if (
                    "benchmark_results" in results
                    and "quantized" in results["benchmark_results"]
                    and task_name in results["benchmark_results"]["quantized"]
                    and metric_name
                    in results["benchmark_results"]["quantized"][task_name]
                ):
                    quant_score = results["benchmark_results"]["quantized"][task_name][
                        metric_name
                    ]
            ...
            orig_scores_display = [
                score * metric_info["multiplier"] for score in orig_scores
            ]
            quant_scores_display = [
                score * metric_info["multiplier"] for score in quant_scores
            ]
            ...
            if metric_info.get("zero_centered", False):
                max_abs_val = max(
                    abs(min(min(orig_scores_display), min(quant_scores_display))),
                ...
                max_y_bench = (
                    max(max(orig_scores_display), max(quant_scores_display)) * 1.2
                )
                min_y_bench = 0
            ...
            for i, (orig, quant) in enumerate(zip(orig_scores, quant_scores)):
                if orig != 0 and quant != 0:
                    if metric_info.get("zero_centered", False):
                        ...
                        text_y = (
                            max(orig_scores_display[i], quant_scores_display[i])
                            + (max_y_bench - min_y_bench) * 0.05
                        )
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=0.85, right=0.85, hspace=0.5, bottom=0.1)
