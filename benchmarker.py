...
def evaluate_llm_benchmark(
    ...
    try:
        ...
        if few_shot_examples > 0:
            few_shot_prompt = create_few_shot_prompt(dataset, task_config, few_shot_examples)
        ...
        for i, example in enumerate(dataset):
            ...
            response = generate_response(
                ...
                max_length=max_length, temperature=temperature, do_sample=do_sample
            )
...
def create_few_shot_prompt(dataset, task_config: Dict[str, Any], num_examples: int) -> str:
...
def create_prompt(example: Dict[str, Any], task_config: Dict[str, Any], few_shot_prompt: str = "") -> str:
...
def generate_response(
    ...
    max_length: int = 512, temperature: float = 0.1, do_sample: bool = False
) -> str:
...
def extract_prediction(response: str, task_config: Dict[str, Any], example: Dict[str, Any]) -> Optional[Union[int, float]]:
    ...
    if task_config["task_type"] == "classification":
        ...
        if "yes" in response or "true" in response or "equivalent" in response or "duplicate" in response:
            return 1
...
def get_label(example: Dict[str, Any], task_config: Dict[str, Any]) -> Optional[Union[int, float]]:
